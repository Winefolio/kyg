...
export function registerSupabaseTestRoutes(app: Express) {
    ...
    app.get("/api/supabase-test/user/:email", async (req, res) => {
        try {
            ...
            // Find all participants with the given email
            ...
            if (participantError) {
                return res.status(500).json({message: "Error fetching participants", error: participantError.message});
            }
            ...
            // Get responses for all found participants
